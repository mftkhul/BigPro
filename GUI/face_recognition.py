    ...
    def face_recog(self):
        def draw_boundray(img,classifier,scaleFactor,minNeighbors,color,text,clf):
            ...
            for (x,y,w,h) in featuers:
                ...
                cursor.execute("select Nama from student where Nim="+str(id))
                n=cursor.fetchone()
                n="+".join(n)

                cursor.execute("select Absen from student where Nim="+str(id))
                r=cursor.fetchone()
                r="+".join(r)

                cursor.execute("select Nim from student where Nim="+str(id))
                i=cursor.fetchone()
                i="+".join(i)


                if confidence > 77:
                    cv2.putText(img,f"Nama:{n}",(x,y-80),cv2.FONT_HERSHEY_COMPLEX,0.8,(64,15,223),2)
                    cv2.putText(img,f"Absen:{r}",(x,y-55),cv2.FONT_HERSHEY_COMPLEX,0.8,(64,15,223),2)
                    cv2.putText(img,f"Nim:{i}",(x,y-30),cv2.FONT_HERSHEY_COMPLEX,0.8,(64,15,223),2)
